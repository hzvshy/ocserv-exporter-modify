...
func (e *Exporter) updateUsers() {
	...
	for _, user := range users {
		...
		t, err := time.Parse("2006-01-02 15:04", user.ConnectedAt)
		if err != nil {
			log.Warnf("Invalid ConnectedAt format for user %s: %v", user.Username, err)
			continue
		}

		vpnUserTX.WithLabelValues(user.Username, user.RemoteIP, user.MTU, user.VPNIPv4, user.VPNIPv6, user.Device, user.UserAgent, user.ConnectedAt).Set(float64(user.RawTX))
		vpnUserRX.WithLabelValues(user.Username, user.RemoteIP, user.MTU, user.VPNIPv4, user.VPNIPv6, user.Device, user.UserAgent, user.ConnectedAt).Set(float64(user.RawRX))
		vpnUserAverageTX.WithLabelValues(user.Username, user.RemoteIP, user.MTU, user.VPNIPv4, user.VPNIPv6, user.Device, user.UserAgent, user.ConnectedAt).Set(float64(txSpeed))
		vpnUserAverageRX.WithLabelValues(user.Username, user.RemoteIP, user.MTU, user.VPNIPv4, user.VPNIPv6, user.Device, user.UserAgent, user.ConnectedAt).Set(float64(rxSpeed))
		vpnUserStartTime.WithLabelValues(user.Username, user.RemoteIP, user.MTU, user.VPNIPv4, user.VPNIPv6, user.Device, user.UserAgent).Set(float64(t.Unix()))
	}
}
